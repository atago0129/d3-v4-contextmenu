...
var ContextMenu = function () {
  ...
  function ContextMenu(rootGroup) {
    ...
    select(document).on('click', function () {
      if (select(event.target.parentNode).classed('context-menu-unclickable')) {
        return;
      }
  ...
  createClass(ContextMenu, [{
    ...
    value: function render(x, y, itemGroup) {
      ...
      var container = select('body').append('div').style('width', width + 'px').style('height', height + 'px').style('left', x + 'px').style('top', y + 'px').style('position', 'absolute').classed('d3-v4-context-menu-container', true).attr('id', itemGroup.id);
    ...
    value: function removeChildGroup(items) {
      ...
      items.map(function (item) {
        ...
        select('#' + item.childGroup.id).remove();
